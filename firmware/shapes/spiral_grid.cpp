// Spiral Grid Animation - Spiral with perpendicular lines creating a grid/box pattern
// Uses the multi-strip LED display with physical coordinate mapping
// Draws a spiral with cross-lines creating a segmented, gridded appearance
...
const uint8_t NUM_GRID_LINES = 20;  // Number of perpendicular lines along spiral
const float ROTATION_SPEED = 0.02;  // Speed of spiral rotation
const uint8_t LINE_LENGTH = 4;      // Length of perpendicular lines (in pixels)
...
// Draw a perpendicular line at a point on the spiral
void drawPerpendicularLine(float t, float angle, float rotationOffset, Color color) {
  // Calculate the perpendicular angle (90 degrees offset)
  float perpAngle = angle + (3.14159 / 2.0);
  
  // Calculate base position on spiral
  float radiusX = MAX_RADIUS_X * t;
  float radiusY = MAX_RADIUS_Y * t;
  int baseX = CENTER_X + radiusX * cos(angle);
  int basePhysY = CENTER_PHYS_Y + radiusY * sin(angle);
  
  // Draw line perpendicular to the spiral
  for (int offset = -LINE_LENGTH; offset <= LINE_LENGTH; offset++) {
    int lineX = baseX + offset * cos(perpAngle);
    int linePhysY = basePhysY + offset * sin(perpAngle) * (MAX_RADIUS_Y / (float)MAX_RADIUS_X);
    
    if (lineX >= 0 && lineX < GRID_WIDTH && linePhysY >= 0 && linePhysY < TOTAL_PHYSICAL_HEIGHT) {
      setPhysicalPixel(lineX, linePhysY, color.r, color.g, color.b);
    }
  }
}

// Draw the spiral with grid lines
void drawSpiralGrid(float rotationOffset) {
  display.clear();
  
  // First draw the main spiral
  for (uint16_t i = 0; i < SPIRAL_POINTS; i++) {
    ...
    float brightness = 0.4 + (t * 0.6);  // Range from 0.4 to 1.0
    
    // Get color for this point
    Color color = getSpiralColor(t, brightness);
    
    // Draw the point (with some thickness)
  ...
  // Now draw perpendicular grid lines at regular intervals
  for (uint8_t line = 0; line < NUM_GRID_LINES; line++) {
    // Calculate position along spiral for this grid line
    float t = (float)line / NUM_GRID_LINES;
    
    // Skip very center (too crowded)
    if (t < 0.15) continue;
    ...
    float brightness = 0.5 + (t * 0.5);
    Color color = getSpiralColor(t, brightness);
    
    // Make grid lines slightly brighter/whiter
    color.r = min(255, (int)(color.r * 1.2));
    color.g = min(255, (int)(color.g * 1.2));
    color.b = min(255, (int)(color.b * 1.2));
    
    // Draw perpendicular line
    drawPerpendicularLine(t, angle, rotationOffset, color);
  }
...
void setup() {
  ...
  display.setBrightness(25);
    ...
    Serial.println("=== SPIRAL GRID ===");
    Serial.println("Rotating spiral with perpendicular grid lines");
    Serial.println("Rainbow gradient with box/grid pattern");
...
void loop() {
  ...
  // Draw the spiral with grid lines
  drawSpiralGrid(currentRotation);
