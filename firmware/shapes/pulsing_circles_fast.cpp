...
const float WAVE_SPEED = 0.18;      // Faster speed (more than 2x original)
...
WaveColor getRainbowColor(float phase, float brightness) {
  ...
  // Create a flowing rainbow gradient
  // Phase goes from 0.0 to 1.0 as wave expands/contracts
  float hue = fmod(phase * 360.0 + millis() / 20.0, 360.0); // Rotating rainbow
